...
<div class="rounded-lg border border-gray-600 bg-gray-800 shadow-sm">
  <div class="px-5 py-3 border-b border-gray-700">
    <h2 class="text-base font-semibold text-gray-200">Product List</h2>
    <p class="text-sm text-gray-300 mt-1">Manage products sold in the catalog</p>
  </div>
  ...
  <div class="p-5">
    ...
    <?php if (empty($products)): ?>
      <div class="text-center py-8">
        ...
        <h3 class="text-lg font-medium text-gray-200 mb-2">No products found</h3>
        <p class="text-gray-300 mb-4">Start adding your first product</p>
        <a href="<?= site_url('admin/product_management/create'); ?>" class="inline-flex items-center rounded-md bg-sky-600 text-white px-4 py-2 text-sm font-semibold hover:bg-sky-700">
          Add First Product
        </a>
      </div>
            ...
            <?php foreach ($products as $product): ?>
              <tr class="hover:bg-gray-750">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-gray-200">#<?= $product['id']; ?></div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm font-medium text-gray-200"><?= htmlspecialchars($product['name']); ?></div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <?php if (!empty($product['image_url'])): ?>
                    <img src="<?= base_url($product['image_url']); ?>" alt="<?= htmlspecialchars($product['name']); ?>" class="h-12 w-12 rounded-lg object-cover border border-gray-600">
                  <?php else: ?>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    <a href="<?= site_url('admin/product_management/edit/' . $product['id']); ?>"
                       class="inline-flex items-center px-3 py-1 rounded-md text-sm font-medium text-blue-300 bg-blue-900/50 hover:bg-blue-900/70">
                    ...
                    <button onclick="confirmDelete(<?= $product['id']; ?>, '<?= htmlspecialchars(addslashes($product['name'])); ?>')"
                            class="inline-flex items-center px-3 py-1 rounded-md text-sm font-medium text-red-300 bg-red-900/50 hover:bg-red-900/70">
                      <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
