...
<div class="space-y-6">
  ...
  <div class="rounded-lg border border-gray-700 bg-gray-800 shadow-sm">
    <div class="px-5 py-3 border-b border-gray-700">
      <h2 class="text-base font-semibold text-slate-100">Telegram Bot Settings</h2>
      <p class="text-sm text-gray-400 mt-1">Enter bot token and chat ID to receive order notifications through Telegram.</p>
    </div>

    <div class="p-5">
      <form action="<?= site_url('admin/settings'); ?>" method="post" class="space-y-5">
      <div>
        <label for="telegram_bot_token" class="block text-sm font-medium text-gray-300">Telegram Bot Token</label>
        <input
          type="text"
          id="telegram_bot_token"
          name="telegram_bot_token"
          value="<?= htmlspecialchars($bot); ?>"
          class="mt-1 w-full rounded-md border border-gray-600 bg-gray-700 px-3 py-2 text-slate-100 shadow-sm focus:border-sky-500 focus:ring-2 focus:ring-sky-500"
          placeholder="e.g. 1234567890:AA...-...."
          required
          minlength="20"
        />
        <p class="mt-1 text-xs text-gray-400">Get token from @BotFather on Telegram.</p>
      </div>

      <div>
        <label for="telegram_chat_id" class="block text-sm font-medium text-gray-300">Telegram Chat ID</label>
        <input
          type="text"
          id="telegram_chat_id"
          ...
          minlength="3"
        />
        <p class="mt-1 text-xs text-gray-400">Use user ID, group, or channel where bot is member/admin.</p>
      </div>

      <div>
        <label for="ad_slider_interval" class="block text-sm font-medium text-gray-300">Ad Slider Interval (seconds)</label>
        <input
          type="number"
          ...
          max="60"
        />
        <p class="mt-1 text-xs text-gray-400">Time interval for ad image rotation in seconds (1-60 seconds).</p>
      </div>

      <div>
        <label for="whatsapp_phone" class="block text-sm font-medium text-gray-300">WhatsApp Number</label>
        <input
          type="text"
          id="whatsapp_phone"
          name="whatsapp_phone"
          ...
          maxlength="20"
        />
        <p class="mt-1 text-xs text-gray-400">WhatsApp number for product inquiries (use international format, e.g. +60143218026).</p>
      </div>
