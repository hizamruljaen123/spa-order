...
<head>
  ...
  <title><?= isset($title) ? $title : 'Log Masuk Admin' ?></title>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 min-h-screen">
    <section class="hidden lg:block relative">
      ...
      <div class="relative z-10 p-8 flex h-full items-end">
        <div class="text-white max-w-lg">
          ...
          <p class="mt-2 text-sm text-white/80">Portal pengurusan tempahan & relaksasi.</p>
        </div>
      </div>
    </section>
    <section class="flex items-center justify-center p-6">
      <div class="w-full max-w-md bg-white rounded-xl shadow-xl ring-1 ring-gray-200 p-8">
        <h1 class="text-2xl font-semibold text-gray-900">Log Masuk Admin</h1>
        ...
        <?= form_open('login', ['class' => 'mt-6 space-y-5']); ?>
          ...
          <div>
            <div class="relative mt-2">
              ...
              <label for="password" class="pointer-events-none absolute left-11 top-1/2 -translate-y-1/2 text-sm text-gray-500 transition-all peer-focus:top-2 peer-focus:text-xs peer-focus:text-brand peer-placeholder-shown:text-sm peer-[&:not(:placeholder-shown)]:top-2 peer-[&:not(:placeholder-shown)]:text-xs peer-[&:not(:placeholder-shown)]:text-brand">Kata Laluan</label>
              <button type="button" id="togglePassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none" aria-label="Tunjukkan kata laluan">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          ...
          <div class="flex items-center gap-3">
            <button type="submit" class="inline-flex items-center justify-center rounded-md bg-brand px-4 py-2 text-white font-semibold shadow-sm hover:bg-indigo-600 focus:outline-none">Log Masuk</button>
          ...
          (function(){
            ...
            if (btn && input) {
              btn.addEventListener('click', function(){
                ...
                this.setAttribute('aria-label', isPwd ? 'Sembunyikan kata laluan' : 'Tunjukkan kata laluan');
              });
            }
          })();
        </script>
        ...
        <p class="mt-6 text-sm text-gray-500">
          Kelayakan lalai awal: <strong>admin</strong> / <strong>admin123</strong>. Sila tukar dalam pangkalan data selepas log masuk.
        </p>
        <p class="mt-2 text-xs text-gray-400">Lupa kata laluan? Hubungi pentadbir sistem.</p>
      </div>
    </section>
  </div>
</body>
