...
<div class="rounded-lg border border-gray-200 bg-white shadow-sm">
  <div class="px-5 py-3 border-b border-gray-200">
    <h2 class="text-base font-semibold text-gray-900"><?= isset($product) ? 'Edit Product' : 'Add New Product'; ?></h2>
  </div>
  <div class="p-6">
    <?php echo form_open(isset($product) ? 'admin/product_management/update/' . $product['id'] : 'admin/product_management/store', ['enctype' => 'multipart/form-data', 'class' => 'grid grid-cols-1 md:grid-cols-2 gap-4']); ?>
      ...
      <div class="md:col-span-2">
        ...
        <p class="mt-1 text-xs text-gray-500">Allowed file types: jpg, jpeg, png, gif. Maximum size: 2MB. Upload image directly from your computer.</p>
      ...
      <?php if (isset($product) && !empty($product['image_url'])): ?>
        <div class="md:col-span-2">
          ...
          <div class="flex items-center gap-3 mt-1">
            <img src="<?= base_url($product['image_url']); ?>" alt="Product Image" class="h-20 w-20 rounded object-cover border border-gray-200">
            <span class="text-xs text-gray-500">Leave image field empty to keep current image.</span>
          </div>
        </div>
      <?php endif; ?>
