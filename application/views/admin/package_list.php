...
<div class="rounded-lg border border-gray-700 bg-gray-800 shadow-sm">
  <div class="px-5 py-3 border-b border-gray-700">
    <h2 class="text-base font-semibold text-slate-100">Package List (APITT)</h2>
  </div>
  <div class="p-0">
    <div class="overflow-x-auto">
      <table class="min-w-full table-auto">
        ...
        <?php if (!empty($packages)): ?>
          <?php $i = 1; foreach ($packages as $p): ?>
            ...
            <tr class="text-sm align-top">
              ...
              <td class="px-5 py-3">
                <?php if (false): ?>
                  <form method="post" action="<?= site_url('admin/package/edit/' . (isset($p->token) ? $p->token : (int)$p->id)); ?>" class="grid grid-cols-1 gap-2">
                    <input type="text" name="name" class="rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" value="<?= htmlspecialchars($p->name); ?>" required minlength="2">
                <?php else: ?>
                  <div class="flex items-center gap-2">
                    <div class="font-medium text-slate-100"><?= htmlspecialchars($p->name); ?></div>
                    <?php if (!empty($p->is_deleted)): ?>
                      <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-900/50 text-red-300">Sudah Dihapus</span>
                    <?php endif; ?>
                  </div>
                <?php endif; ?>
              </td>
              ...
              <td class="px-5 py-3">
                <?php if (false): ?>
                  <input type="text" name="category" class="rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" value="<?= htmlspecialchars($p->category ?? ''); ?>" required>
                <?php else: ?>
                  <div class="text-gray-400"><?= htmlspecialchars($p->category ?? '-'); ?></div>
                <?php endif; ?>
              </td>

              <!-- Hands -->
              <td class="px-5 py-3 text-center text-gray-300">
                <?php if (false): ?>
                  <select name="hands" class="rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100">
              ...
              <td class="px-5 py-3 text-center text-gray-300">
                <?php if (false): ?>
                  <input type="number" name="duration" class="w-24 text-center rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" value="<?= (int)($p->duration ?? 0); ?>" min="15" step="5" required>
                <?php else: ?>
              ...
              <td class="px-5 py-3 text-right text-gray-300">
                <?php if (false): ?>
                  <input type="number" name="price_in_call" class="w-28 text-right rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" value="<?= (float)($p->price_in_call ?? 0); ?>" min="0" step="1" required>
                <?php else: ?>
              ...
              <td class="px-5 py-3 text-center">
                <?php if (false): ?>
                  <input type="text" name="currency" class="w-20 text-center rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" value="<?= htmlspecialchars($p->currency ?? 'RM'); ?>" maxlength="10" required>
                <?php else: ?>
                  <span class="inline-flex items-center px-2 py-1 rounded bg-gray-700 text-gray-200 text-xs"><?= htmlspecialchars($p->currency ?? 'RM'); ?></span>
                <?php endif; ?>
              </td>
              ...
              <td class="px-5 py-3">
                <?php if (false): ?>
                  <textarea name="description" class="w-full rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100" rows="1"><?= htmlspecialchars($p->description ?? ''); ?></textarea>
                <?php else: ?>
                  <div class="text-gray-400 truncate max-w-[340px]"><?= htmlspecialchars($p->description ?? '-'); ?></div>
                <?php endif; ?>
              </td>
              ...
              <td class="px-5 py-3 text-right">
                ...
                <?php else: ?>
                    <?php if (empty($p->is_deleted)): ?>
                      <a class="inline-flex items-center rounded-md border border-gray-600 bg-gray-700 text-gray-200 px-3 py-1.5 text-xs hover:bg-gray-600" href="<?= site_url('admin/package/edit/' . (isset($p->token) ? $p->token : (int)$p->id)); ?>">Edit</a>
                      <a class="inline-flex items-center rounded-md border border-red-800 bg-gray-700 text-red-400 px-3 py-1.5 text-xs hover:bg-red-900/50" href="<?= site_url('admin/package/delete/' . (isset($p->token) ? $p->token : (int)$p->id)); ?>" onclick="return confirm('Hapus paket ini? (Data booking historis akan tetap terjaga)');">Delete</a>
                    <?php else: ?>
                      <span class="text-xs text-gray-400 bg-gray-700 px-2 py-1 rounded">Tidak Aktif</span>
                      <a class="inline-flex items-center rounded-md border border-green-800 bg-gray-700 text-green-400 px-3 py-1.5 text-xs hover:bg-green-900/50" href="<?= site_url('admin/package/restore/' . (isset($p->token) ? $p->token : (int)$p->id)); ?>" onclick="return confirm('Pulihkan paket ini?');">Restore</a>
                    <?php endif; ?>
                <?php endif; ?>
              </td>
            </tr>
        ...
        <?php else: ?>
          <tr>
            <td colspan="9" class="px-5 py-6 text-center text-gray-400">No package data available.</td>
          </tr>
        <?php endif; ?>
        </tbody>
      </table>
...
<!-- Edit Package Modal -->
<div id="pkgEditModalOverlay" class="fixed inset-0 bg-black/60 <?= $hasEdit ? '' : 'hidden'; ?>"></div>
<div id="pkgEditModal"
     role="dialog"
     aria-modal="true"
  ...
  <div class="w-full max-w-2xl bg-gray-800 rounded-xl shadow-xl ring-1 ring-gray-600">
    <div class="px-6 py-4 border-b border-gray-700 flex items-center justify-between">
      <h3 class="text-lg font-semibold text-slate-100">Edit Package</h3>
      <button type="button" class="text-gray-400 hover:text-gray-200" onclick="window.pkgEditClose()" aria-label="Close">
      ...
      <?php if ($hasEdit && isset($editItem)): ?>
        <form method="post" action="<?= site_url('admin/package/edit/' . $editToken); ?>" class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            <label for="edit_name" class="block text-sm font-medium text-gray-300">Package Name</label>
          ...
          <div>
            <label for="edit_hands" class="block text-sm font-medium text-gray-300">Number of Therapists</label>
            <select id="edit_hands" name="hands" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100">
          ...
          <div>
            <label for="edit_duration" class="block text-sm font-medium text-gray-300">Duration (minutes)</label>
            <input id="edit_duration" name="duration" type="number" min="15" step="5" required value="<?= (int)($editItem->duration ?? 0); ?>"
                   class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-center text-slate-100">
          </div>
          ...
          <div>
            <label for="edit_price_in_call" class="block text-sm font-medium text-gray-300">Price</label>
          ...
          <div class="md:col-span-2 flex justify-end gap-3 mt-2">
            ...
            <a href="<?= site_url('admin/packages'); ?>" class="inline-flex items-center rounded-md border border-gray-600 bg-gray-700 text-gray-200 px-4 py-2 text-sm hover:bg-gray-600">Cancel</a>
          </div>
        </form>
      <?php else: ?>
        <p class="text-sm text-gray-400">Package data not found.</p>
      <?php endif; ?>
    </div>
  </div>
</div>
...
<div id="pkgCreateModalOverlay" class="fixed inset-0 bg-black/60 hidden"></div>
<div id="pkgCreateModal"
  ...
  <div class="w-full max-w-2xl bg-gray-800 rounded-xl shadow-xl ring-1 ring-gray-600">
    <div class="px-6 py-4 border-b border-gray-700 flex items-center justify-between">
      <h3 class="text-lg font-semibold text-slate-100">Add Package</h3>
      <button type="button" class="text-gray-400 hover:text-gray-200" onclick="window.pkgCreateClose()" aria-label="Close">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
        </svg>
      </button>
    ...
    <div class="p-6">
      <form method="post" action="<?= site_url('admin/package/create'); ?>" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2">
          <label for="create_name" class="block text-sm font-medium text-gray-300">Package Name</label>
        ...
        <div>
          <label for="create_hands" class="block text-sm font-medium text-gray-300">Number of Therapists</label>
          <select id="create_hands" name="hands" required class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-slate-100">
        ...
        <div>
          <label for="create_duration" class="block text-sm font-medium text-gray-300">Duration (minutes)</label>
        ...
        <div>
          <label for="create_price_in_call" class="block text-sm font-medium text-gray-300">Price</label>
        ...
        <div class="md:col-span-2 flex justify-end gap-3 mt-2">
          ...
          <button type="button" class="inline-flex items-center rounded-md border border-gray-600 bg-gray-700 text-gray-200 px-4 py-2 text-sm hover:bg-gray-600" onclick="window.pkgCreateClose()">Cancel</button>
        </div>
      </form>
    </div>
  </div>
